(function () {
  ...
  describe('iui-alert-service', function () {
    ...
    // Let's test first if the getCurrent method is called first, then the add method is called second
    describe('when the getCurrent(\'alert_agent99\') method is called', function () {
      beforeEach(function () {
        result = iuiAlertService.getCurrent(alertId);
      });

      it('should create a placement if a placement does not exist and should return an empty object {}', function () {
    ...
    // Now let's test if the add method is called first, then the getCurrent method is called second
    describe('when the add method is called with the id `alert_agent99` and '+
      'the object { message: \'Pardon me while I get my shoe phone.\', type: \'danger\', canClose: true}', function () {
      beforeEach(function () {
        iuiAlertService.add(alertId, alert);
      });
    ...
    describe('when the add method is called with the id `alert_agent99` and '+
      'the object { message: \'Ah, it\'s the old... trick.\', type: \'success\', activeFor: 4000, templateUrl: \'/template/template.html\'}', function () {
      beforeEach(function () {
      ...
      describe('AND when the getCurrent(alertId) method is called', function () {
        ...
        it('should return an alert with the templateUrl set to /template/template.html', function() {
          expect(result.alert.templateUrl).toBe('/template/template.html');
        });
    ...
    describe('when getCurrent is called for both \'alert_angent99\' and \'alert_chief\'', function () {
      beforeEach(function () {
